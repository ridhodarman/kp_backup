            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <h3><small>Berkas yang belum dikonfirmasi</small></h3>
                        <table id="example1" class="table table-bordered table-striped" style="width: 100%">
                            ...
                            <tr style="background-color: #fa8072">
                                ...
                                <th><center>Tanggal selesai proses oleh Edval SC & Jasa</th></center>
                            ...
                            <tbody>
                            <?php
                                $query = mysql_query("SELECT * FROM berkas WHERE status=4") or die(mysql_error());
                                $no = 1;
                                while($result = mysql_fetch_assoc($query)){                                 
                                    ...
                                    $caritanggalmasuk = mysql_query("SELECT * FROM tracking_berkas where no_notifikasi='$notif' && id_keterangan=4") or die(mysql_error());
                                    ...
                                    echo '<td><center>
                                    <a href="act/konfirmasiberkas.php?notifikasi='.$result['no_notifikasi'].'"><button type="button" class="btn btn-sm btn-success">Konfirmasi</button></a>
                                    </center></td>'; 
                                    echo '</tr>';
                                    $no++;
                                }
                        ...
                        <h3><small>Berkas yang sudah dikonfirmasi</small></h3>
                            ...
                            <tr style="background-color: lightgreen">
                                ...
                                <th><center>Judul</th></center>
                                <th><center>Unit Kerja</th></center>
                                <th><center>Tanggal pekerjaan</th></center>
                                <th><center>Telah diberikan kepada user</th></center>
                                <th><center>Laporan Absensi & kwitansi</th></center>
                                <th><center>Action</th></center>
                            </tr>
                            </thead>
                            <tbody>
                            <?php
                                $query = mysql_query("SELECT * FROM berkas WHERE status=5") or die(mysql_error());
                                $no = 1;
                                while($result = mysql_fetch_assoc($query)){                                 
                                    echo '<tr>';
                                    echo '<td><center>'.$no.'</center></td>';  
                                    ...
                                    echo '<td><center>'.date('d F Y', strtotime($result['tgl_pekerjaan_dimulai'])).' s/d '.date('d F Y', strtotime($result['tgl_pekerjaan_selesai'])).'</center></td>';

                                    $notif=$result['no_notifikasi'];

                                    $cekuser = mysql_query("SELECT * FROM tracking_berkas where no_notifikasi='$notif' && id_keterangan=6") or die(mysql_error());
                                    $cekresensi = mysql_query("SELECT * FROM tracking_berkas where no_notifikasi='$notif' && id_keterangan=7") or die(mysql_error());



                                    if(mysql_num_rows($cekuser) == 0){
                                    echo '<td><center>
                                    <a href="act/proseskeuser.php?notifikasi='.$result['no_notifikasi'].'"><button type="button" class="btn btn-sm btn-warning">OK</button></a>
                                    </center></td>'; 
                                    }
                                    else{
                                        echo '<td><center><font color="darkgreen"><b>DONE</b></font>... ';
                                        if (mysql_num_rows($cekresensi) == 0) {
                                            echo '<a href="act/cancelprosesuser.php?notifikasi='.$result['no_notifikasi'].'"><button type="button" class="btn btn-sm btn-default" onclick="return confirm(\'Cancel proses ?\')">X</button></a>';
                                        }
                                        echo '</center></td>';
                                    }


                                    if(mysql_num_rows($cekresensi) == 0 && mysql_num_rows($cekuser) == 1){  
                                        echo '<td><center>
                                        <a href="act/prosesberkas.php?notifikasi='.$result['no_notifikasi'].'"><button type="button" class="btn btn-sm btn-warning" onclick="return confirm(\'Apakah kelengkapan laporan berkas sudah cukup (absensi & kwitansi) ?\')">OK</button></a>
                                        </center></td>'; 
                                        }
                                    else  {
                                        if (mysql_num_rows($cekresensi) == 1){
                                        echo '<td><center><font color="darkgreen"><b>DONE</b></font>... ';
                                        echo '<a href="act/cancelprosesberkas.php?notifikasi='.$result['no_notifikasi'].'"><button type="button" class="btn btn-sm btn-default" onclick="return confirm(\'Cancel proses ?\')">X</button></a>';
                                        echo '</center></td>';
                                        }
                                        else {
                                            echo "<td>...</td>";
                                    ...
                                    if(mysql_num_rows($cekuser) == 0 && mysql_num_rows($cekresensi) == 0){
                                        echo '<td><center>
                                       <a href="act/cancelkonfirmasiberkas.php?notifikasi='.$result['no_notifikasi'].'"><button type="button" class="btn btn-sm btn-danger" onclick="return confirm(\'Batalkan konfirmasi berkas ?\')">Batalkan konfirmasi</button></a>';
                                    }                                    

                                    else {
                                        echo '<td><center>..';
                                        if(mysql_num_rows($cekuser) == 1 && mysql_num_rows($cekresensi) == 1){
                                        echo '
                                            <a href="act/proseskeeval2.php?notifikasi='.$result['no_notifikasi'].'"><button type="button" class="btn btn-sm btn-success">Proses Ke Eval SC & Jasa</button></a>
                                            </td></center>
                                        ';
                                        }   
                                    }                                    
