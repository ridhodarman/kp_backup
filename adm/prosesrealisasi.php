                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <h3 class="page-header">
                            Berkas yang dikirimkan ke Eval SC & Jasa Untuk Dievaluasi dan Proses RAB realisasi
                        </h3>
                        ...
                        <table id="example1" class="table table-bordered table-striped" style="width: 100%">
                            ...
                            <?php
                                ...
                                while($result = mysql_fetch_assoc($query)){                                 
                                    ...
                                    echo '<td><center>
                                    <a href="act/batalproses.php?notifikasi='.$result['no_notifikasi'].'"><button type="button" class="btn btn-sm btn-danger" onclick="return confirm(\'Cancel proses ?\')">Cancel proses</button></a>
                                    </center></td>'; 
                                    echo '</tr>';
                                    $no++;
                                }
                        ...
                        <table id="example2" class="table table-bordered table-striped" style="width: 100%">
                            ...
                            <tbody>
                            <?php
                                $query = mysql_query("SELECT * FROM berkas WHERE status>6 && status<=8") or die(mysql_error());
                                $no = 1;
                                while($result = mysql_fetch_assoc($query)){                                 
                                    echo '<tr>';
                                    echo '<td><center>'.$no.'</center></td>';  
                                    ...
                                    $notif=$result['no_notifikasi'];
                                    $cektanggal = mysql_query("SELECT * FROM tracking_berkas where no_notifikasi='$notif' && id_keterangan=7") or die(mysql_error());
                                    $cekkonfirm = mysql_query("SELECT * FROM tracking_berkas where no_notifikasi='$notif' && id_keterangan=8") or die(mysql_error());
                                    $cekproses = mysql_query("SELECT * FROM tracking_berkas where no_notifikasi='$notif' && id_keterangan=9") or die(mysql_error());
                                
                                    while($data = mysql_fetch_assoc($cektanggal)){                                 
                                    echo '<td><center>'.date('d F Y -- H:i', strtotime($data['tanggal'])).'</center></td>';
                                    }

                                    if (mysql_num_rows($cekkonfirm) == 1 && mysql_num_rows($cekproses) == 0) {
                                        while($data2 = mysql_fetch_assoc($cekkonfirm)){
                                        echo '<td><center>Telah dikonfirmasi oleh Bidang Eval SC & Jasa pada '.date('d F Y - H:i', strtotime($data2['tanggal'])).'</center></td>';
                                        }
                                    }
                                    else if (mysql_num_rows($cekproses) == 1) {
                                        while($data3 = mysql_fetch_assoc($cekproses)){
                                        echo '<td><center>RAB realisasi telah selesai diproses Eval SC & Jasa pada tanggal '.date('d F Y - H:i', strtotime($data3['tanggal'])).'</center></td>';
                                        }
                                    }
                                    echo '</center></td>';

                                    if (mysql_num_rows($cekproses) == 1) {
                                        echo '<td><a href="act/konfirmasi2.php?notifikasi='.$result['no_notifikasi'].'"><button type="button" class="btn btn-sm btn-success">Konfirmasi</button></a></td>';   
                                         }
                                    else {
                                        echo "<td>...</td>";
                                    }  
