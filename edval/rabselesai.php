                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <h1 class="page-header"><small><b>
                            RAB realisasi yang telah selesai diproses
                        </b></small></h1>
                        <h3><small><b>RAB realisasi yang sudah difinalisasi</b></small></h3>
                        <table id="example1" class="table table-bordered table-striped" style="width: 100%">
                            ...
                            <tr style="background-color: #fa8072">
                                ...
                                <th><center>Tanggal Finalisasi</th></center>
                                <th><center>Action</th></center>
                            </tr>
                            </thead>
                            <?php
                                $query = mysql_query("SELECT * FROM berkas WHERE status=8") or die(mysql_error());
                                    $no = 1;
                                    while($result = mysql_fetch_assoc($query)){                                 
                                        ...
                                        $notif=$result['no_notifikasi'];
                                        $cari = mysql_query("SELECT * FROM tracking_berkas where no_notifikasi='$notif' && id_keterangan=4") or die(mysql_error());
                                        while($tanggal = mysql_fetch_assoc($cari)){
                                            $t=date('d F Y -- H:i', strtotime($tanggal['tanggal']));
                                            echo '<td><center>'.$t.'</center></td>';
                                        }
                                        echo '<td><center>
                                        <a href="act/hapusfinalisasi2.php?notifikasi='.$result['no_notifikasi'].'" onclick="return confirm(\'Batalkan finalisasi berkas ?)\')"><button type="button" class="btn btn-sm btn-danger">Cancel</button></a>
                                        </center></td>'; 
                                        echo '</tr>';
                                        $no++;
                                    }
                        ...
                        <h3><small><b>RAB yang sudah dikonfirmasi kembali ke meja ADM</b></small></h1>
                        <table id="example2" class="table table-bordered table-striped" style="width: 100%">
                            ...
                            <tr style="background-color: lightgreen">
                                ...
                                <th><center>Judul</th></center>
                                <th><center>Unit Kerja</th></center>
                                <th><center>Tanggal RAB realisasi dibuat</th></center>
                            ...
                            <?php
                                $query = mysql_query("SELECT * FROM berkas WHERE status>=9") or die(mysql_error());
                                    ...
                                    while($result = mysql_fetch_assoc($query)){                                 
                                        ...
                                        $cari = mysql_query("SELECT * FROM rab where no_notifikasi='$notif'") or die(mysql_error());
                                        while($tanggal = mysql_fetch_assoc($cari)){
                                            $t=date('d F Y', strtotime($tanggal['tgl_rab_realisasi']));
                                            echo '<td><center>'.$t.'</center></td>';
                                        }
                                        $konfirmasi = mysql_query("SELECT * FROM tracking_berkas where no_notifikasi='$notif' && id_keterangan=10") or die(mysql_error());
                                        while($tanggal2 = mysql_fetch_assoc($konfirmasi)){
                                            $d=date('d F Y -- H:i', strtotime($tanggal2['tanggal']));
                                            echo '<td><center>Telah diberikan oleh ADM kepada user pada '.$d.'</center></td>';
                                        }
                                        echo '</tr>';
                                        $no++;
                                    }
