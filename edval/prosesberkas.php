                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <h1 class="page-header">
                            <small><b> Berkas SPK yang sedang di proses </b></small>
                        </h1>
                            ...
                            <tr style="background-color: #fa8072">
                                ...
                                <th><center>Status RAB Sementara</th></center>
                            </tr>
                            </thead>
                            <?php
                                ...
                                while($result = mysql_fetch_assoc($query)){                                 
                                    ...
                                    $caritanggalmasuk = mysql_query("SELECT * FROM tracking_berkas where no_notifikasi='$notif' && id_keterangan=3") or die(mysql_error());
                                    ...
                                    echo '<td><center>
                                    <a href="detailberkas.php?notifikasi='.$result['no_notifikasi'].'"><button type="button" class="btn btn-sm btn-primary">View Details</button></a> /
                                    <a href="act/hapusprosesberkas.php?notifikasi='.$result['no_notifikasi'].'" onclick="return confirm(\'Yakin batalkan konfirmasi berkas ?\')"><button type="button" class="btn btn-sm btn-danger">X</button></a>
                                    </center></td>';
                                    $sql5 = mysql_query("SELECT * FROM rab where no_notifikasi='$notif'") or die(mysql_error());
                                    if(mysql_num_rows($sql5) == 0){
                                    echo '<td><font color="gray">RAB Sementara belum selesai diproses</font></td>';
                                    }
                                    else{
                                        while($hasil2 = mysql_fetch_assoc($sql5)){
                                            $no_rab=$hasil2['no_rab'];
                                            $tgl_rab_dibuat=$hasil2['tgl_rab_dibuat'];
                                            $rab_awal=$hasil2['rab_awal'];
                                        }
                                        if($no_rab==null || $tgl_rab_dibuat==null || $rab_awal==null){
                                        echo '<td>RAB belum selesai diproses</td>';    
                                        }
                                        else{
                                            echo '<td><a href="act/finalisasiberkas.php?notifikasi='.$result['no_notifikasi'].'"><button type="button" class="btn btn-sm btn-success" onclick="return confirm(\'Finalisasi berkas ? (Berkas difinalisasi jika sudah ditanda tangani oleh kepala bidang)\')">Finalisasi</button></a>
                                                </center></td>';
                                        }
                                    } 
                                    echo '</tr>';
